...
void DGUSScreenHandlerMKS::sendInfoScreen_P(const void *line1, const void *line2, const void *line3, const void *line4, uint16_t language) {
...
void DGUSScreenHandlerMKS::DGUSLCD_SetUint8(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  dgus.writeVariable(VP_T_Fan0_ON, thermalManager.fan_speed[0] > 0 ? (uint16_t)1 : (uint16_t)0); // Свое, ОТПРАВКА
}
...
// }
  ...
  void DGUSScreenHandler::sdFileSelected(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    if (filelist.isDir()) { 
      ...
      // Send print filename
      dgus.WriteString(VP_SD_Print_Filename, filelist.filename(), VP_SD_FileName_LEN);
      // dgusdisplay.writeVariable(VP_SD_Print_Filename, filelist.filename(), VP_SD_FileName_LEN, true);
    #endif

    // Setup Confirmation screen
    file_to_print = touched_nr;
    dgus.WriteString(VP_SD_FileName_Select, filelist.filename(), VP_SD_FileName_LEN*2);
  ...
  void DGUSScreenHandler::sdStartPrint(DGUS_VP_Variable &var, void *val_ptr) {
    ...
    uint16_t print_pr = 0;
    dgus.writeVariable(VP_PrintProgress_Percentage, print_pr); //чтобы в начале не прыгало на 100%
  ...
  void DGUSScreenHandler::sdSendFilename(DGUS_VP_Variable& var) {
    ...
    dgus.WriteString(var.VP, tmpFilename, VP_SD_FileName_LEN);
  ...
  void DGUSScreenHandlerMKS::WriteMsg(uint16_t VP, FSTR_P message, uint16_t data_size){
    ...
    if(data_size == 0){
      dgus.WriteString(VP, message, VP_Status_LEN);
    } else {
      dgus.WriteString(VP, message, data_size);
    }
  ...
  void DGUSScreenHandlerMKS::Error(FSTR_P message, bool is_critical){
    ...
    if(is_critical){
      ...
      has_error = true;
      dgus.writeVariable(VP_ERROR_STATUS, (uint16_t)1);
    }
  }
  ...
  void DGUSScreenHandler::sdCardRemoved() {
    // if (current_screenID == DGUSLCD_SCREEN_SDFILELIST  //ЗАКОМЕНТИЛ
    ...
    // Свое
    if(ExtUI::isPrintingFromMedia()){
      dgus.WriteString(VP_PrintStatus, GET_TEXT_F(MSG_NO_MEDIA), VP_Status_LEN);
      gotoScreen(MKSLCD_SCREEN_PrintDone);
    } else if (current_screenID == MKSLCD_SCREEN_CHOOSE_FILE ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_1 ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_2 ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_3 ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_FOLDER ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_FOLDER_1 ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_FOLDER_2 ||
       current_screenID == MKSLCD_SCREEN_CHOOSE_FILE_FOLDER_3 ||
       current_screenID == MKSLCD_SCREEN_PRINT_CONFIRM
  ...
  void DGUSScreenHandlerMKS::SDPrintingFinished() { //TODO почему то не работает
    ...
    dgus.WriteString(VP_PrintStatus, GET_TEXT_F(MSG_PRINT_DONE), VP_Status_LEN); 
    gotoScreen(MKSLCD_SCREEN_PrintDone);
...
void DGUSScreenHandlerMKS::Cool_Down(DGUS_VP_Variable &var, void *val_ptr) {      
  ...
  dgus.writeVariable(VP_T_Fan0_ON, (uint16_t)0);   // uint16_t обязателен, чтобы было 00 01, а не 01 00(uint8_t).
...
void DGUSScreenHandler::handleFanSpeedChange(DGUS_VP_Variable &var, void *val_ptr) {
  ...
  dgus.writeVariable(VP_T_Fan0_ON, thermalManager.fan_speed[0] > 0 ? (uint16_t)1 : (uint16_t)0);
  forceCompleteUpdate(); 
}
...
void DGUSScreenHandlerMKS::FilamentLoadUnload(DGUS_VP_Variable &var, void *val_ptr, const int filamentDir) {
  ...
  // }
    ...
    if (hotend_too_cold) {
      ...
      dgus.WriteString(VP_FILAMENT_CHANGE_STATUS, GET_TEXT_F(MSG_HOTEND_TOO_COLD), VP_SD_FileName_LEN);
      // gotoScreen(DGUS_SCREEN_POPUP);// заменить на вывод текста что очень холодно
    }
  #endif
...
void DGUSScreenHandlerMKS::DGUS_Runout_Idle() {
    ...
    switch (runout_mks.runout_status) {
      ...
      case RUNOUT_STATUS:
        ...
        sendInfoScreen_P(F("NOTICE"), nullptr, F("Please change filament!"), nullptr, true, true, true, true);
        //setupConfirmAction(nullptr);
